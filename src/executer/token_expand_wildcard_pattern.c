...
/*   Updated: 2022/08/01 21:18:27 by minkyeki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	match_pattern(struct dirent *file, struct stat *buf, t_list **match, t_pattern *pattern)
{
	if (pattern->prefix->f_is_empty(pattern->prefix) && pattern->suffix->f_is_empty(pattern->suffix))
	{
	...
	else if (ft_strncmp(file->d_name, pattern->prefix->text, pattern->prefix->text_len) == 0 \
			&& ft_strncmp_reverse(file->d_name, pattern->suffix->text, pattern->suffix->text_len) == 0)
	{
		if (pattern->remainder->text[0] == '/' && S_ISDIR(buf->st_mode))
			ft_lstadd_back(match, ft_lstnew(new_token(file->d_name)));
		else if (pattern->remainder->text[0] != '/')
